class DropBoxController{
  constructor(){
    ...
    this.listFile = document.querySelector('#list-of-files-and-directories')
    this.fileName = document.querySelector('.filename');
    this.navEl = document.querySelector('#browse-location')
    this.duration = document.querySelector('.timeleft');
    this.rename = document.querySelector('#btn-rename')
    this.newFolder = document.querySelector('#btn-new-folder');
    this.delete = document.querySelector('#btn-delete');
    this.connect();
    this.openFolder();
    this.initEvents();
  ...
  getSelection(){
    return this.listFile.querySelectorAll('.selected');
  }
  remoteTask(){
  ...
  initEvents(){
    this.newFolder.addEventListener('click', event =>{
      ...
      this.remoteTask()
        .then(responses => {
          responses.forEach( response =>{
            ...
            if(response.key){
              this.firebaseRef().child(response.key).remove()
            }
          })
        })
        .catch(err =>{
    ...
    this.rename.addEventListener('click', event =>{
    ...
    this.listFile.addEventListener('selectionchange', event =>{
      switch(this.getSelection().length){
        ...
        case 1 :
          ...
          this.rename.style.display = 'block';
        break;
        default:
          this.delete.style.display = 'block';
          this.rename.style.display = 'none';
    ...
    this.inputFileEl.addEventListener('change', event =>{
      ...
      this.uploadTask(files)
        .then(responses => {
          responses.forEach(response =>{
            this.firebaseRef().push().set(response.files)
          })
          this.uploadComplete();
        })
        .catch(err =>{
  ...
  connect(){
    let firebaseConfig = {
      apiKey: "AIzaSyA3GarxAXkfNINMcoG3BrG7xFHX-wOmPOA",
      authDomain: "dropbox-clone-777f5.firebaseapp.com",
      databaseURL: "https://dropbox-clone-777f5.firebaseio.com",
      projectId: "dropbox-clone-777f5",
      storageBucket: "dropbox-clone-777f5.appspot.com",
      messagingSenderId: "385810167772",
      appId: "1:385810167772:web:c896a05e5c4afaa57f0119",
      measurementId: "G-HD3W35BDTK"
    };
  ...
  uploadTask(files){
    ...
    convertFiles.forEach(file =>{
      let form = new FormData();
      let config = {
        onUploadProgress: event => this.progress(event, file),
      }
      form.append('files', file);
      this.uploadTime = Date.now();
      filesReceived.push(new Promise((resolve, reject)=>{
        axios.post('/uploads', form, config)
          .then(response=> {
            resolve(response.data)
          })
          .catch(err => {
    ...
    return Promise.all(filesReceived);
  }
  ...
  progress(event, file){
    ...
    let pastTime = Date.now() - this.uploadTime;
    let timeleft = ((100 - percentage) * pastTime) / percentage;
    
    this.progressBar.style.width = `${percentage}%`;
    this.fileName.innerHTML = file.name;
    this.duration.innerHTML = this.formatTime(timeleft);
  }
  ...
  getReadFile(){
    ...
    this.firebaseRef().on('value', snapshot =>{
      this.listFile.innerHTML = '';
      snapshot.forEach(item =>{
        const key = item.key;
        const data = item.val();
        if(data.type){
          this.listFile.appendChild(this.getfileView(data, key));
        }
      })
    })
  }
  ...
  openFolder(){
    if(this.lastFolder){
      this.firebaseRef(this.lastFolder).off('value');
    }
    this.renderNav()
    this.getReadFile();
  }
  ...
  initEventsLi(li){
    ...
    li.addEventListener('click', event =>{
      if(event.shiftKey){
        const first = this.listFile.querySelector('.selected');
        if(first){
          let start;
          let end;
          const childs = li.parentElement.childNodes;
          ...
          let index = [start, end].sort();
          childs.forEach((el, ind)=>{
            if(ind >= index[0] && ind <= index[1]){
              el.classList.add('selected');
            }
          });
          this.listFile.dispatchEvent(this.onselectionchange);
          return true;
        }
      }
      if(!event.ctrlKey){
        this.listFile.querySelectorAll('li.selected')
          .forEach(item =>{
